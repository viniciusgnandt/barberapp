...
export default function Profile() {
  ...
  const handleSavePwd = async () => {
    ...
    setSavingPwd(true);
    const r = await Users.updateMe({ currentPassword: pwdForm.currentPassword, newPassword: pwdForm.newPassword });
    setSavingPwd(false);
    if (r.ok) {
  ...
  return (
    <div className="max-w-xl space-y-6 animate-fade-up">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Conta</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Informações pessoais e segurança</p>
      </div>
          ...
          >
            ...
            <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
              {uploadingAvatar
                ...
                : <><Camera size={16} className="text-white" /><span className="text-white text-[10px] mt-1">Alterar</span></>
              }
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 space-y-4">
        ...
        <Input label="Nome completo" value={nameForm.name} onChange={e => setNameForm({ name: e.target.value })} placeholder="Seu nome" />
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 space-y-4">
        ...
        <Input label="Senha atual"          type="password" placeholder="••••••••" value={pwdForm.currentPassword} onChange={e => setPwdForm(f => ({ ...f, currentPassword: e.target.value }))} />
        <Input label="Nova senha"           type="password" placeholder="••••••••" value={pwdForm.newPassword}     onChange={e => setPwdForm(f => ({ ...f, newPassword: e.target.value }))} />
        <Input label="Confirmar nova senha" type="password" placeholder="••••••••" value={pwdForm.confirmPassword} onChange={e => setPwdForm(f => ({ ...f, confirmPassword: e.target.value }))} />
        {pwdError && <p className="text-sm text-red-500 bg-red-50 dark:bg-red-900/20 px-3 py-2 rounded-lg">{pwdError}</p>}
        <div className="flex justify-end">
